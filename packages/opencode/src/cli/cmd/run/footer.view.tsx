...
export function RunFooterView(props: RunFooterViewProps) {
  ...
  const queuedKeymapShortcut = useKeymapSelector(
    (keymap: OpenTuiKeymap) =>
      formatKeySequence(
        keymap
  ...
  const queuedShortcut = createMemo(() => queuedKeymapShortcut() || "ctrl+x q")
  ...
  const promptFromPreview = (item: FooterQueuedPrompt): RunPrompt => ({
    text: item.text,
    parts: [],
    queueID: item.id,
    queued: true,
  ...
  const removeQueued = async (item: FooterQueuedPrompt) => {
    const removed = await queueControl()?.remove(item.id)
    if (removed !== undefined) return
    await props.onQueuedRemove(item.id)
  }

  const editQueued = async (item: FooterQueuedPrompt) => {
    const control = queueControl()
    const prompt = control?.get(item.id) ?? (control?.load ? await control.load(item.id) : promptFromPreview(item))
    if (!prompt) return
    await removeQueued(item)
    closePanel()
    queueMicrotask(() => composer.replacePrompt(prompt))
  }
                      ...
                      <Switch>
                        ...
                        <Match when={selectingQueued()}>
                          <RunQueuedPromptSelectBody
                            ...
                            onClose={closePanel}
                            onDelete={(item) => void removeQueued(item)}
