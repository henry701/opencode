...
export const getUsage = (input: { model: Provider.Model; usage: Usage; metadata?: ProviderMetadata }) => {
  ...
  if (process.env["OPENCODE_CACHE_AUDIT"]) {
    const count = tokens.input + tokens.cache.read + tokens.cache.write
    const hit = count > 0 ? ((tokens.cache.read / count) * 100).toFixed(1) : "0.0"
    console.error(
      `[CACHE] ${input.model.id}  input=${count} (cache_read=${tokens.cache.read} cache_write=${tokens.cache.write} new=${tokens.input})  hit=${hit}%  output=${tokens.output}  total=${tokens.total ?? 0}`,
    )
  }
  const costInfo =
