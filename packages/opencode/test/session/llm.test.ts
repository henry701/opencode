...
const collectEvents = (input: LLM.StreamInput, hooks?: LLM.StreamHooks) =>
  LLM.Service.use((svc) => svc.stream(input, hooks).pipe(Stream.runCollect)).pipe(
    Effect.map((events) => Array.from(events)),
  )

// drainWith builds an isolated runtime so custom replacements fully own LLM and
// its transitive deps.
const drainWith = (layer: Layer.Layer<LLM.Service>, input: LLM.StreamInput, hooks?: LLM.StreamHooks) =>
  Effect.gen(function* () {
    const ctx = yield* InstanceRef
    if (!ctx) return yield* Effect.die("InstanceRef not provided")
    return yield* Effect.promise(() =>
      Effect.runPromise(
        LLM.Service.use((svc) => svc.stream(input, hooks).pipe(Stream.runDrain)).pipe(
...
describe("session.llm.stream", () => {
  ...
  it.instance(
    "reports prepared context outside the provider event stream",
    () =>
      Effect.gen(function* () {
        const fixture = loadFixture(alibabaQwenFixture.providerID, alibabaQwenFixture.modelID)
        const request = waitRequest(
        ...
        expect(prepared[0]?.systemPrompt).toBe(providerSystemPrompt(capture.body))
        const toolDefs = JSON.parse(prepared[0]?.toolDefs ?? "{}") as Record<string, unknown>
        ...
        expect(events.map((event) => event.type)).not.toContain("context-metadata")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const prepared: LLM.PreparedContext[] = []
        yield* drainWith(
          llmLayerWithExecutor({ flags: { experimentalNativeLlm: true } }),
          {
            user: {
        ...
        const capture = yield* Effect.promise(() => request)
        expect(prepared[0]?.systemPrompt).toContain("You are a helpful assistant.")
        expect(prepared[0]?.toolDefs).toBeUndefined()
        expect(capture.url.pathname.endsWith("/responses")).toBe(true)
