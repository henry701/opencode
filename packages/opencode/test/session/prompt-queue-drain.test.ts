...
const queueLayer = Layer.mergeAll(Session.defaultLayer, SessionPromptQueue.defaultLayer, Bus.layer)
...
describe("session queue drain pause (http)", () => {
  const headersFor = (directory: string) => ({
  ...
  itHttp.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = headersFor(test.directory)
        ...
        expect(yield* Effect.promise(() => pause.json())).toBe(true)

        const resume = yield* Effect.promise(() =>
          Promise.resolve(
            Server.Default().app.request(`/session/${session.id}/queue/drain-resume`, {
              method: "POST",
              headers: { "x-opencode-directory": test.directory },
            }),
          ),
        ...
        expect(yield* Effect.promise(() => resume.json())).toBe(true)
      }),
    { git: true },
  )
    ...
    () =>
      Effect.gen(function* () {
        ...
        const res = yield* Effect.promise(() =>
          Promise.resolve(
            Server.Default().app.request(`/session/ses_missing/queue/drain-pause`, {
              method: "POST",
              headers: { "x-opencode-directory": test.directory },
            }),
          ),
