...
import { hydrateRemoteQueue } from "@/cli/cmd/run/runtime"
...
describe("run runtime queue", () => {
  test("hydrates remote queued prompts into the footer on attach", async () => {
    const ui = footer()
  ...
  test("ignores remote queue hydration failures", async () => {
    const ui = footer()
    ...
    await hydrateRemoteQueue({
      ...
      footer: ui.api,
      listQueue: async () => {
        throw new Error("missing queue route")
      },
    })
