...
describe("detectServerProtocol", () => {
  test("prefers the current session endpoint when both API generations exist", async () => {
    const fetcher = mockFetch((input) => {
      const path = new URL(input instanceof Request ? input.url : input).pathname
      if (path === "/api/session") return Promise.resolve(json({ data: [], cursor: {} }))
  ...
  test("recognizes the current API from its session envelope", async () => {
    const fetcher = mockFetch((input) => {
      const path = new URL(input instanceof Request ? input.url : input).pathname
      if (path === "/api/session") return Promise.resolve(json({ data: [], cursor: { next: undefined } }))
